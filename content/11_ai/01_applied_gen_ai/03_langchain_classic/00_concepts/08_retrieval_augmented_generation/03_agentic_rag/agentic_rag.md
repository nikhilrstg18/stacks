---
title: "Agentic RAG"
slug: "11_ai/01_applied_gen_ai/03_langchain_classic/00_concepts/08_retrieval_augmented_generation/03_agentic_rag"
stack: "GenAI"
...
📌 Here the output was not concluded even though agent went into multiple **ReAct** loops but was not able to run the tools in a desired sequence.

📌 `AgenticRAG` is **suitable** when you are leaving it upto the agents to select the tool

❓ What if you want Agent to follow a certain path, then `AgenticRAG` is **not suitable**

We use `GraphRAG` when I want Agents to follow a certain path

This is resolved using the next evolution using [Lang
Graph](https://www.langchain.com/langgraph). Let's dive into next module LangGraph.
