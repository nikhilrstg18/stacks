---
title: "Agentic RAG"
slug: "11_ai/01_applied_gen_ai/03_langchain/00_concepts/04_rag/03_agentic_rag"
stack: "GenAI"
...
- An `Agent` in LangChain uses a reasoning loop `ReAct` to decide which tools to call, often making multiple retrievals before finalizing an answer.
- `RetrievalQA` is simpler: it retrieves once and directly answers based on context.
- Agents are better for multi‑step reasoning; `RetrievalQA` is faster and cleaner for straightforward Q&A.

## Agent Vs RetrievalQA

|          | Agent                                                                            | RetrievalQA                                       |
| -------- | -------------------------------------------------------------------------------- | ------------------------------------------------- |
| Workflow | Think → Act → Observe → Repeat.                                                  | Retrieve once → pass context → answer.            |
| Behavior | May call retriever multiple times, reason about results, then finalize.          | Single-shot, clean, efficient.                    |
| Case     | Complex queries needing multi-step reasoning, tool orchestration, or validation. | Straightforward Q&A where one retrieval suffices. |
| Tradeoff | Slower, verbose logs, sometimes redundant retrievals                             | Less flexible, no multi-step reasoning            |
...
- **Traditional RAG**: LLM retrieves chunks from a knowledge base and generates answers.
- **Agentic RAG**: Adds _agency_ — the model can plan tasks, choose retrieval strategies, validate outputs, and refine reasoning.
- **Benefit**: More robust, adaptive, and trustworthy applications (e.g., enterprise assistants, compliance-aware chatbots).

Your agent is running (~3 times) because of the `ReAct` reasoning loop.
It keeps re-calling the retriever until it feels confident.
You can fix this by limiting iterations, switching to a simpler chain, or adjusting the agent’s instructions.

- **Agentic RAG** = RAG + agent planning + guardrails + human oversight.
- **Scenario**: Customer support assistant that retrieves docs, sanitizes queries, and generates safe answers.
- **Steps**: Setup retrieval → define tools → add guardrails → build agent → enforce human review.
